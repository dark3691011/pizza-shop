...
function UpdateProduct({match}) {
    ...
    const updateProduct = product => {
        axios.put(`${global.config.server.link}/product/${match.params.id}`,{
            name: product.name
        })
        .then(res =>{
        ...
        <div className="update__product">
            <form onSubmit={submitHandle} id="update__product__form">
                <div className="form__group">
                    ...
                    <input type="text" id="name" name="name" onChange={e => setProduct({...product, name: e.target.value})} value={product.name}/>
                </div>
